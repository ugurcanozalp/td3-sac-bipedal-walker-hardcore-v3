...
\begin{itemize}
	...
	ahead of it with lidar measurements and cannot observe behind, as illustrated in Figure \ref{eqn:partial_obs_pitfall}. 
...
\begin{itemize}
	...
	In first case, the terminal flag causes instability since next step's value is not used in value update, since it is not a logical terminal, just time up indicator.
...
\begin{figure}
	...
	\begin{subfigure}{.35\textwidth}
		\centering
		\includegraphics[width=0.85\linewidth]{figures/nets/critic.png}
...
In our application, sequence of incoming vectors is passed though single layer vanilla LSTM layer with 96 dimensional hidden state. 
...
In our application, sequence of incoming vectors is passed through single layer pre-layer normalized transformer with 192 dimensional feed forward layer with GELU activation. 
...
Agent cannot exceed 280 points in any simulation with TD3 (Figure \ref{fig:td3_scatter_ep_rewards}) but it exceeds 300 points with SAC (Figure \ref{fig:sac_scatter_ep_rewards}). Also, moving average points is also higher when SAC policy is used. 
