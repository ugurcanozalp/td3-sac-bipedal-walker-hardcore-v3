...
def train_ddpg(env, agent, n_episodes=5000, max_t=1000, populate_episode=20, trainer_name='type_x'):
    ...
    for i_episode in range(1, n_episodes+1):
        ...
        if score >=200:
            torch.save(agent.train_actor.state_dict(), './models/'+trainer_name+'_ckpt_actor.pth')
            torch.save(agent.train_critic.state_dict(), './models/'+trainer_namer+'_ckpt_critic.pth')
            break
    return scores
