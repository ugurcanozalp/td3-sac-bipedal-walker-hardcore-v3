...
class TD3Agent():
    ...
    def __init__(self, Actor, Critic, clip_low, clip_high, state_size=24, action_size=4, update_freq=int(2),
            lr=3e-4, weight_decay=0, gamma=0.98, tau=0.005, batch_size=128, buffer_size=int(5e5)):
        ...
        self.noise_generator = DecayingOrnsteinUhlenbeckNoise(mu=np.zeros(action_size), theta=3.0, sigma=0.9, dt=0.04, sigma_decay=0.999)
    ...
    def load_ckpt(self, model_type, env_type, prefix='last'):
        actor_file = os.path.join("models", "td3", env_type, "_".join([prefix, model_type, "actor.pth"]))
        critic_1_file = os.path.join("models", "td3", env_type, "_".join([prefix, model_type, "critic_1.pth"]))
        critic_2_file = os.path.join("models", "td3", env_type, "_".join([prefix, model_type, "critic_2.pth"]))
        try:
