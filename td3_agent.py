...
class TD3Agent():
    ...
    def __init__(self, Actor, Critic, clip_low, clip_high, state_size=24, action_size=4, update_freq=int(2),
            lr=1e-3, gamma=0.99, tau=0.005, batch_size=128, buffer_size=int(5e4)):
        ...
        self.noise_generator = OrnsteinUhlenbeckNoise(mu=np.zeros(action_size), theta=0.2, sigma=0.1)
        self.target_noise = GaussianNoise(mu=np.zeros(action_size), sigma=0.2)
