...
try:
    agent.train_actor.load_state_dict(torch.load(os.path.join('models', 'best_mlp_ckpt_actor.pth'), map_location={'cuda:0': 'cpu'}))
    agent.train_critic.load_state_dict(torch.load(os.path.join('models', 'best_mlp_ckpt_critic.pth'), map_location={'cuda:0': 'cpu'}))
except:
