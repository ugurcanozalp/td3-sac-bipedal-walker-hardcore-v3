...
class MyBiLSTM(nn.Module):
    def __init__(self, input_size, hidden_size=64, batch_first=True):
        super(MyBiLSTM, self).__init__()
        self.lstm = nn.LSTM(input_size, hidden_size=hidden_size, batch_first=batch_first, bidirectional=True, num_layers=1, dropout=0)
        self.lstm.bias_hh_l0.data.fill_(-0.2) # force lstm to output to depend on only last state at the initialization.
        self.lstm.bias_hh_l0_reverse.data.fill_(-0.2) # force lstm to output to depend more on last state at the initialization.
...
class Critic(nn.Module):
    ...
    def __init__(self, state_dim=24, action_dim=4):
        ...
        self.state_encoder = MyBiLSTM(input_size=self.state_dim, hidden_size=64, batch_first=True)
