...
class PriotrizedReplayBuffer:
    """Simle priotrized experience replay buffer for deep reinforcement algorithms."""
    def __init__(self, action_size, buffer_size, batch_size, device):
        self.action_size = action_size
        self.memory = deque(maxlen=buffer_size)  
        self.prios_memory = deque(maxlen=buffer_size)  
        self.device = device
    ...
    def add(self, state, action, reward, next_state, done):
        ...
        max_prio = max(self.prios_memory)
        self.prios_memory.append(max_prio)
